...
export default function WholeMealDealView({
    initBody,
    getTopMealDeals,
}: Props) {
            ...
            {body.map((mealDeal) => (
                ...
                >
                    <div className="flex gap-4">
                        <Item
                            image={mealDeal.drink_image}
                            name={mealDeal.drink_name}
                        />
                        <span>+</span>
                        <Item
                            image={mealDeal.snack_image}
                            name={mealDeal.snack_name}
                        />
                        <span>+</span>
                        <Item
                            image={mealDeal.main_image}
                            name={mealDeal.main_name}
                        />
                    </div>
                    <p className="mt-4 text-2xl">
                        <span className="font-bold">
                            {mealDeal.vote_count} vote
                            {mealDeal.vote_count === 1 ? "" : "s"}
                        </span>
                        <InlineVoteCaster
                            drinkId={mealDeal.drink_id}
                            snackId={mealDeal.snack_id}
                            mainId={mealDeal.main_id}
                        />
                    </p>
                </li>
            ))}
