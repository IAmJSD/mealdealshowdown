...
async function loadTescoPage(pageNumber: number) {
    return fetch("https://api.tesco.com/shoppingexperience", {
        method: "POST",
        headers: {
        ...
        if (!res.ok) {
            throw new Error(
                `Failed to fetch tesco meal deal data: ${res.status} ${await res.text()}`,
            );
        }
...
export async function getTescoMealDealData(): Promise<MealDealData> {
    ...
    while (p1.pageInformation.totalCount > allPages.length * 300) {
        allPages.length === 0
            ? allPages.push(p1)
            : allPages.push(await loadTescoPage(allPages.length + 1));
    }
    ...
    for (const product of allProducts) {
        ...
        const arr =
            product.restrictedDelivery === "Main"
                ? data.mains
                : product.restrictedDelivery === "Snack"
                  ? data.snacks
                  : data.drinks;
        arr.push({
            id: product.id,
